...
async def process_workplace(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"▫️ Ожидайте звонок или сообщение в ближайшее время\n"
        f"▫️ Мы подробно ответим на все интересующие Вас вопросы\n\n"
        f"📑 Оформление возможно день в день\n"
        f"✍🏻 Если хочется ускорить процесс - пишите нашему менеджеру @easyworkmanager",
        reply_markup=ReplyKeyboardRemove(),
        parse_mode="HTML"
    )
    await state.clear()
